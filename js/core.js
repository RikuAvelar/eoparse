...
var host_port = QueryString.HOST_PORT || 5060;
...
function Person(e, p) {
    ...
    if (this.Class == "" || split.length > 1) {
        if (smnPetsList.indexOf(petsName) > -1) {
            ...
            this.Class = "MCH";
            this.isPet = true;
        } else if (geoPetsList.indexOf(petsName) > -1) {
            this.Job = "AVA";
            this.Class = "GEO";
            this.isPet = true;
    ...
    try {
        ...
        {
            if (this.Job == "0" || this.Job == "AVA" || this.Job == 'LMB')
...
Combatant.prototype.AttachPets = function() {
    ...
    for (var i in this.Combatant) {
        ...
        if (this.Combatant[i].Job == "AVA" || this.Combatant[i].Job === 'LMB') {
